...
class AccessToken_test : public testing::Test {
 protected:
  ...
  void testAccessToken(std::string expected, AccessToken key) {
    ...
    EXPECT_EQ(k6.message_.messages[AccessToken::kJoinChannel],
              key.message_.messages[AccessToken::kJoinChannel]);
  ...
  void testAccessTokenWithIntUid() {
    ...
    key.message_.messages[AccessToken::kJoinChannel] = expiredTs;
  ...
  void testAccessTokenWithErrorUid() {
    ...
    key.message_.salt = 1;
    key.message_.ts = 1111111;
    key.AddPrivilege(AccessToken::kJoinChannel, 100);
    testAccessToken(expected, key);
  }
